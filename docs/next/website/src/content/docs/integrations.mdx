---
title: Integrations
description: Install Herdr integrations for Pi, Claude Code, Codex, OpenCode, Hermes Agent, and GitHub Copilot CLI.
---
...
The hook reports agent state to the local Herdr socket when GitHub Copilot CLI exposes hook events.

Herdr uses `~/.copilot` by default, or `COPILOT_HOME` when set. The Copilot config directory must already exist. Install writes `hooks/herdr-agent-state.sh` (executable) and `hooks/herdr-agent-state.json` containing the hook configuration. Uninstall deletes both of these files.
